...
- [x] **HEALTH-02**: `list_accounts` response includes per-account health fields: `status`, `last_error`, `last_error_at`, `last_connected_at`. *(Done — Plan 13-02)*
- [x] **HEALTH-03**: Health metadata is detailed enough for an agent to explain to a user *why* an account is unavailable (e.g. distinguish "retrying after network drop" from "credentials need to be fixed"). *(Done — Plan 13-02 via reconnecting `attempt`/`next_retry_at` + suspended `humanReason` stock string)*
...
| HEALTH-02 | Phase 13 | Done (Plan 13-02, commit 84ae905) |
| HEALTH-03 | Phase 13 | Done (Plan 13-02, commit 84ae905) |
